...
const Clap = ({ story }) => {
  const [clapCount, setClapCount] = useState(story?.clapsCount);
  const [isClapped, setIsClapped] = useState(story.isClapped);
  ...
  const handleClap = async () => {
    if (isAuthor) { 
      console.error("You cannot clap your own story");
    ...
    try {
      ...
      if (response.ok) {
        setClapCount(prevCount => isClapped ? prevCount - 1 : prevCount + 1);
        setIsClapped(prevIsClapped => !prevIsClapped);
      } else {
        console.error("Error updating clap count");
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="clap_container">
      <svg
        onClick={ location.pathname === `/story/${story.id}` ? handleClap : undefined }
      ...
      <div className="clapCount">
        {formatCount(clapCount === undefined ? story.clapsCount : clapCount)}
      </div>
      {showAuthorNote && <p className="authorNote">Authors cannot clap their own stories.</p>}
    </div>
  );
};
